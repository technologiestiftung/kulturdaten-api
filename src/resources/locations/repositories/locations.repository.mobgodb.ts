...
export class MongoDBLocationsRepository implements LocationsRepository {
	...
	async updateLocationVisibility(locationIdentifier: string, visibility: string): Promise<ExecutionResult> {
		const locations = await this.dbConnector.locations();
		...
		return Promise.resolve({success: result.modifiedCount === 1});
	}
